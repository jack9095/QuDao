...
public class ToastUtils {
    ...
    private static int yOffset = (int) (64 * BaseApplication.getAppContext().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate = (LayoutInflater) BaseApplication.getAppContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(@StringRes int resId, int duration) {
        show(BaseApplication.getAppContext().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(BaseApplication.getAppContext().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        cancel();
        if (customView != null) {
            sToast = new Toast(BaseApplication.getAppContext());
